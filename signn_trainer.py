...
class signn_trainer():
    ...
    def __init__(self, dataset_path, model_path, epochs, steps_per_epoch, batch_size,
                 shuffle, shuffle_buffer_size, split_ratio, validation_steps):
        ...
        self.dataset_parser = ddg.deepsig_dataset_generator(self.dataset_path, snr=[10])
        self.train_samples = int((self.split_ratio[0]*self.dataset_parser.get_total_samples()/self.batch_size))
        self.validation_samples = int((self.split_ratio[1]*self.dataset_parser.get_total_samples()/self.batch_size))
        self.__init_dataset()
    ...
    def __init_dataset(self):
        ...
        self.validation_dataset = self.validation_dataset.prefetch(buffer_size=self.batch_size)
...
def argument_parser():
    ...
    parser.add_argument("--split-ratio", default="0.8/0.2", nargs='+', dest="split_ratio",
                        action="store", type=float,
                        help='Set the train/validation portions. (Default: %(default)s)')
    parser.add_argument("--validation-steps", dest="validation_steps",
                        type=int, default=2, help="Set the number of validation steps.")
    return parser
...
def main(trainer=signn_trainer, args=None):
    ...
    t = trainer(dataset_path=args.dataset_path, model_path=args.model_path,
                ...
                split_ratio=args.split_ratio, validation_steps=args.validation_steps)
    t.train()
